...
REM ==== Intervalo do DIA ANTERIOR (com ajuste de fuso Brasil UTC-3 → UTC) ====
for /f "delims=" %%I in ('
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$d=(Get-Date).AddDays(-1).Date; $d=$d.AddHours(3); $d.ToString('yyyy-MM-dd HH:mm:ss')"
') do set "FROM=%%~I"
for /f "delims=" %%I in ('
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$d=(Get-Date).AddDays(-1).Date; $d=$d.AddHours(26).AddMinutes(59).AddSeconds(59); $d.ToString('yyyy-MM-dd HH:mm:ss')"
') do set "TO=%%~I"
echo FROM=!FROM!
echo TO=!TO!

set "LOGFILE=wialon_consigaz_log.txt"
